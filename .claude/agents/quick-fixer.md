...
### Step 2: 기존 패턴 및 재사용 가능 코드 검색 (신규)
1. 유사한 문제를 해결한 기존 코드 검색
2. 재사용 가능한 유틸리티나 컴포넌트 확인
3. 동일한 패턴으로 수정된 사례 찾기

**재사용 검색**:
```bash
# 유사 에러 처리 패턴
grep -r "try.*catch\|handleError" src/ | head -10

...
1. **프로젝트 분석 필수**: 초기화 전 반드시 프로젝트 구조 분석 (절대 건너뛰지 않음)
2. **기존 설정 확인**: .specify/ 디렉토리 존재 시 사용자 확인 필요
3. **아키텍처 감지**: 자동 감지 후 신뢰도 확인, 모호할 경우 사용자 선택 요청
4. **AskUserQuestion 사용**: 다음 단계 제안 시 필수 (텍스트 질문 금지)
5. **한글 출력**: 모든 초기화 메시지는 반드시 한글로 작성

---

## 필수 확인 사항

- [ ] 기존 .specify/ 디렉토리 존재 여부 (덮어쓰기 주의)
- [ ] constitution.md 존재 여부
- [ ] package.json 또는 프로젝트 구조 파일 존재
- [ ] Git 저장소 여부 (권장)
...
1. 프로젝트 구조 분석을 사용하여 초기 분석 수행
2. 기존 패턴과 설정을 확인하여 아키텍처 감지
3. Constitution 및 설정 파일 생성
4. 개발 환경 설정 가이드 및 권장사항 제공
5. 진행하기 전에 사용자에게 설정 확인 요청

**절대로 프로젝트 분석 단계를 건너뛰지 마세요.**

# 🎯 Model Optimization & Context7 Integration Guide

Claude Workflows v2.1에서 도입된 지능형 모델 최적화 시스템 사용 가이드입니다.

## 📚 목차

1. [개요](#개요)
2. [모델 전략](#모델-전략)
3. [Context7 통합](#context7-통합)
4. [설정 방법](#설정-방법)
5. [사용 예시](#사용-예시)
6. [비용 최적화](#비용-최적화)
7. [문제 해결](#문제-해결)

## 개요

### 시스템 아키텍처

```yaml
모델 계층:
  Opus: 최고 품질, 복잡한 기획 및 분석
  Sonnet: 균형잡힌 성능, 일반적인 작업
  Haiku: 빠른 처리, 간단한 작업

Context7:
  역할: 프로젝트 전체 컨텍스트 로딩
  활용: Plan 단계, 재사용성 검색
  토큰: 최대 3,000 (선택적 로딩)
```

### 핵심 특징

- **자동 모델 선택**: 작업 복잡도에 따른 최적 모델 자동 선택
- **쿼터 관리**: Opus 한도 도달 시 자동 폴백
- **Context7 통합**: 필요 시 프로젝트 컨텍스트 자동 로드
- **비용 최적화**: 토큰 사용량 40-50% 절감

## 모델 전략

### 단계별 기본 모델

| 단계 | 기본 모델 | 업그레이드 조건 | Context7 |
|------|----------|----------------|----------|
| **Specify** | Opus | - | ❌ |
| **Plan** | Sonnet | 복잡도 > 12 → Opus | ✅ (복잡도 > 10) |
| **Clarify** | Sonnet | - | ❌ |
| **Tasks** | Sonnet | - | ❌ |
| **Implement** | Sonnet | - | ❌ |
| **Minor** | Sonnet | 단순 수정 → Haiku | ❌ |
| **Micro** | Haiku | - | ❌ |

### 복잡도 점수 계산

```yaml
복잡도 요소:
  파일_개수:
    1-3개: +1점
    4-10개: +3점
    11개 이상: +5점

  아키텍처_영향:
    기존 패턴: +1점
    새 패턴: +4점
    아키텍처 변경: +6점

  Breaking_Changes:
    Public API: +5점
    폴더 구조: +4점
    Import 경로: +3점

  Cross_Cutting:
    여러 레이어: +4점
    여러 모듈: +3점
    단일 모듈: +1점

모델 선택:
  0-5점: Haiku
  6-11점: Sonnet
  12점 이상: Opus
```

## Context7 통합

### 활성화 조건

Context7은 다음 조건에서 자동 활성화됩니다:

```yaml
자동 활성화:
  - 복잡도 점수 > 10
  - feature_type: "cross_cutting"
  - 아키텍처 변경 감지
  - 새 API 엔드포인트 추가

수동 활성화:
  - 플래그: --use-context7
  - 명령어: /enable-context7
```

### 선택적 로딩 전략

```yaml
Context7 로딩 구성 (최대 3,000 토큰):
  프로젝트 구조: 500 토큰
    - src/ 디렉토리 구조
    - 아키텍처 패턴 파악

  관련 컴포넌트: 1,500 토큰
    - features/*/api/*
    - shared/hooks/*
    - 유사 구현 패턴

  기존 패턴: 1,500 토큰
    - 재사용 가능 모듈
    - 공통 유틸리티

  아키텍처 규칙: 500 토큰
    - constitution.md
    - architecture.json
```

### 통합된 Skills/Agents

| 컴포넌트 | Context7 사용 | 토큰 예산 | 용도 |
|----------|-------------|----------|------|
| **reusability-enforcer** | ✅ | 2,000 | 재사용 모듈 검색 |
| **major-plan** | ✅ | 3,000 | 기술 계획 수립 |
| **api-integration** | ✅ | 1,500 | API 패턴 검색 |
| **component-creation** | ✅ | 1,000 | 컴포넌트 템플릿 |

## 설정 방법

### 1. 사용자 전략 선택

`.claude/config/user-preferences.yaml` 편집:

```yaml
model_preferences:
  default_strategy: "balanced"  # quality, balanced, aggressive 중 선택
```

#### 전략별 특징

**Quality (품질 우선)**
- 모든 중요 단계에 Opus 사용
- Context7 항상 활성화
- 토큰 예산: 무제한
- 예상 비용: 높음

**Balanced (균형, 권장)**
- Specify/Plan만 Opus
- Context7 조건부 활성화
- 토큰 예산: 10,000/단계
- 예상 비용: 중간

**Aggressive (비용 최적화)**
- 모든 단계 Sonnet/Haiku
- Context7 비활성화
- 토큰 예산: 5,000/단계
- 예상 비용: 낮음

### 2. Context7 설정

```yaml
context7_preferences:
  default_mode: "auto"  # auto, manual, never
  max_tokens: 3000
  auto_activation_threshold: 10  # 복잡도 점수
  loading_strategy: "selective"  # selective, comprehensive
```

### 3. 알림 설정

```yaml
notifications:
  model_switches:
    enabled: true
    verbose: false  # 상세 정보 표시

  context7_usage:
    enabled: true
    show_token_count: true

  cost_tracking:
    enabled: true
    show_cumulative: true
```

## 사용 예시

### 1. 기본 사용

```bash
# 자동 모델 선택 및 Context7
/major user-authentication

# 출력:
ℹ️ Using Opus for specify stage (critical requirements)
🔍 Context7 activated for plan stage (complexity: 14)
ℹ️ Using Sonnet for tasks stage
```

### 2. 모델 강제 지정

```bash
# 특정 모델 강제 사용
/major user-auth --model=sonnet

# Opus 우선 사용 요청
/major user-auth --prefer-opus

# Context7 강제 활성화
/major user-auth --use-context7
```

### 3. 비용 최적화 모드

```bash
# 비용 최적화 우선
/major user-auth --optimize-cost

# Context7 비활성화
/major user-auth --no-context7

# Haiku 사용 (매우 간단한 작업)
/micro fix-typo
```

### 4. Minor 워크플로우 자동 다운그레이드

```bash
/minor fix-button-color

# 자동 감지:
ℹ️ Using Haiku for simple fix (cost optimization)
  - Change scope: 3 lines in 1 file
  - Type: style change
```

## 비용 최적화

### 토큰 사용량 비교

| 워크플로우 | 기존 (All Opus) | 최적화 후 | 절감률 |
|-----------|----------------|-----------|--------|
| Major (복잡) | 50,000 | 15,000 | 70% |
| Major (단순) | 30,000 | 8,000 | 73% |
| Minor | 20,000 | 5,000 | 75% |
| Micro | 10,000 | 1,500 | 85% |

### 비용 추적

```bash
# 일일 사용량 확인
cat .claude/metrics/daily/$(date +%Y-%m-%d).json

# 출력 예시:
{
  "opus_requests": 5,
  "sonnet_requests": 23,
  "haiku_requests": 45,
  "total_tokens": 125000,
  "estimated_cost": "$8.50",
  "context7_activations": 3
}
```

### 최적화 팁

1. **Opus 절약**
   - Specify/Plan 단계만 사용
   - 복잡도 12점 이상만 활성화
   - 일일 쿼터 관리

2. **Context7 효율화**
   - 선택적 로딩 사용
   - 캐시 활용 (30분 유지)
   - 수동 검색 폴백 준비

3. **Haiku 활용**
   - 타이포 수정
   - 스타일 변경
   - 10줄 이하 수정

## 문제 해결

### Opus 쿼터 초과

**증상:**
```
⚠️ Opus quota exhausted, using Sonnet with enhanced mode
```

**해결책:**
1. 5분 후 자동 재시도
2. Sonnet + 강화 모드 사용
3. 수동으로 `--model=sonnet` 지정

### Context7 연결 실패

**증상:**
```
❌ Context7 connection failed
↩️ Falling back to manual search
```

**해결책:**
1. 수동 검색 모드로 자동 전환
2. Grep/Glob 도구 사용
3. `--no-context7` 플래그로 비활성화

### 토큰 한도 초과

**증상:**
```
⚠️ Context7 token limit reached (3000/3000)
```

**해결책:**
1. 부분 결과로 진행
2. 로딩 범위 축소
3. 선택적 로딩 전략 조정

### 모델 성능 저하

**증상:**
- Haiku/Sonnet 사용 시 품질 저하

**해결책:**
1. `--prefer-opus` 플래그 사용
2. 사용자 전략을 "quality"로 변경
3. 복잡한 작업은 Major 워크플로우 사용

## 모니터링

### 메트릭 수집

```yaml
# .claude/config/user-preferences.yaml
advanced:
  metrics:
    enabled: true
    track_model_usage: true
    track_token_consumption: true
    track_cost_per_workflow: true
    export_path: ".claude/metrics/"
```

### 대시보드 확인

```bash
# 모델별 사용 통계
grep "model_usage" .claude/metrics/daily/*.json | jq

# Context7 활성화 빈도
grep "context7" .claude/metrics/daily/*.json | wc -l

# 평균 토큰 사용량
jq '.total_tokens' .claude/metrics/daily/*.json | awk '{sum+=$1} END {print sum/NR}'
```

## 권장 사항

### DO ✅

- Major 워크플로우는 Opus로 시작
- Context7은 복잡한 작업만 활성화
- 간단한 수정은 Haiku 활용
- 캐시를 통한 중복 로딩 방지
- 정기적인 메트릭 모니터링

### DON'T ❌

- 모든 작업에 Opus 강제 사용
- Context7 상시 활성화
- 캐시 무효화 빈번히 수행
- 토큰 한도 무시
- 폴백 메커니즘 비활성화

## 업데이트 내역

### v2.1.0 (2024-01-07)
- 모델 라우터 시스템 도입
- Context7 선택적 로딩
- 쿼터 인식 폴백
- 사용자 전략 프로파일

### 향후 계획
- 자동 학습 기반 모델 선택
- 팀 공유 쿼터 관리
- 실시간 비용 알림
- A/B 테스트 기반 최적화